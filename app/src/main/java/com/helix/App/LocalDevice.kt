...
object LocalDevice {
    ...
    /** Headset (or host phone) battery via BatteryManager — no ADB. */
    ...
    /** Opens the system screen where the user grants WRITE_SETTINGS. */
    ...
    fun setStayOnWhilePlugged(context: Context, on: Boolean): Boolean {
        ...
        return try {
            ...
            // Global may need WRITE_SECURE_SETTINGS; try System as fallback no-op log
            Log.e(TAG, "setStayOnWhilePlugged failed (may need secure settings)", e)
            false
        }
    }

    /** True if Developer options is unlocked (Settings.Global.development_settings_enabled). */
    ...
    /**
     * Try to flip development_settings_enabled=1 (needs WRITE_SECURE_SETTINGS).
     * Returns true if the flag is on after the call.
     */
    ...
    /**
     * Try to start a settings intent. Returns whether startActivity succeeded
     * (does not guarantee the right UI appeared — Meta may remap some intents).
     */
    fun tryStartSettingsAction(context: Context, action: String, dataUri: String? = null): IntentProbeResult {
        return try {
            ...
            // Prefer matching an activity so we don't "succeed" on implicit nothing
    ...
    /** Common Android + a few Quest-ish settings actions to probe. */
    fun allSettingsProbeActions(context: Context = AppContext.app): List<Pair<String, String?>> {
        ...
        return listOf(
            ...
            "Settings\$DevelopmentSettingsActivity" to null, // handled specially
    ...
    fun tryStartNamedSetting(context: Context, label: String, action: String?): IntentProbeResult {
        if (action == null) {
            // Explicit components sometimes used on AOSP
            val components = when {
        ...
        val data = when (action) {
            ...
            Settings.ACTION_APP_NOTIFICATION_SETTINGS -> null // extra needed
            else -> null
        }
    ...
    /**
     * Main entry for the Open Settings button:
     * 1) If developer options off → try enable (secure) or open About / device info and instruct user
     * 2) If on → open wireless debugging / developer settings
     */
    fun openWirelessDebugging(context: Context = AppContext.app): String {
        ...
        if (!enabled) {
            ...
            if (!flipped) {
                // Open About / device info so user can tap Build number 7×
        ...
        // Prefer wireless debugging intents, then developer options
        val wireless = listOf(
...
object LocalPower {
    ...
    suspend fun cpuSnapshot(): Snap = withContext(Dispatchers.IO) {
        // Prefer no-root /proc/stat (two samples)
        ...
        // Enrich with root utils if available
        var rootExtra = ""
    ...
    suspend fun gpuSnapshot(): Snap = withContext(Dispatchers.IO) {
        // Try no-root kgsl reads (often permission denied on Quest)
